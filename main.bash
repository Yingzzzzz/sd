...
# Ready to make .csv output      
      ...
      sed -i "s|^[^,]*|`date '+%d/%m/%Y %H:%M'`|g" Finallogdone.csv
      awk -F '|' '!x[$8,$9,$10,$11]++' Finallogdone.csv > filename_dedup.csv
      cat filename_dedup.csv |  awk -F"[,]+" '{print $1,$5,$6,$8}' > abc.csv
     # sed -i "s|^[^,]*|`date '+%d/%m/%Y %H:%M'`|g" abc.csv
      cat abc.csv >> WebLog.csv
      cat Finallogtranend1.csv |  awk -F"[,]+" '{print $1,$3,$7}' > bca.csv
   #   sed -i "s|^[^,]*|`date '+%d/%m/%Y %H:%M'`|g" bca.csv
      cat bca.csv >> WebLog.csv
     done
